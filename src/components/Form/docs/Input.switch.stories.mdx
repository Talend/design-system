...
`:readonly` differs from `:disabled` in a major way: a `:readonly` value is sent on submit, not a `:disabled` one. But because `[type="checkbox"]` has no native support for `:readonly`, we implemented a custom behavior using `[type:"hidden"]`. The only drawback is that the value is sent as a string.
		...
		{() => {
			...
			return (
				<Form onSubmit={handleSubmit(setFormData)}>
					...
					<Form.Fieldset legend="Default checked">
						<Form.Switch label="Option b" {...register('option-b')} />
					</Form.Fieldset>
					<Form.Fieldset legend="Read only">
						<Form.Switch label="Option c" readOnly {...register('option-c')} />
					</Form.Fieldset>
					<Form.Fieldset legend="Disabled">
						<Form.Switch label="Option d" disabled {...register('option-d')} />
