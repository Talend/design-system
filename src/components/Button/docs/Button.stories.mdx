...
<Canvas>
	...
	<Story name="primary--icon">
		<Button.Primary onClick={action('clicked')} icon="talend-minus-circle">
			Button with icon
		</Button.Primary>
	</Story>
	...
	<Story
		...
		argTypes={{
			...
			icon: {
				...
				control: {
					type: 'select',
					options: Array.from(document.querySelectorAll('.tc-iconsprovider symbol')).map(symbol => symbol.getAttribute('id')).filter(Boolean),
				},
			},
			loading: {
