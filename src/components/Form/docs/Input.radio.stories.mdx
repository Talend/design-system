...
`:readonly` differs from `:disabled` in a major way: a `:readonly` value is sent on submit, not a `:disabled` one. But because `[type="radio"]` has no native support for `:readonly`, we implemented a custom behavior using `[type:"hidden"]`. The only drawback is that the value is sent as a string.
